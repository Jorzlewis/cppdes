...
app.factory("Aut", function(FBURL, $firebaseAuth, $firebaseObject) {
	...
	var Aut = {
		...
		crearPerfil: function(uid, usuario){
			var perfil = {
				...
				correo_institucional: usuario.email,
				correo_personal: usuario.correo_personal,
				estado: "Abierto",
				fecha_creacion: Firebase.ServerValue.TIMESTAMP,
				//email: usuario.email,
	...
	aut.$onAuth(function(authData) {
		if(authData) {      
	      ...
	      Aut.usuario.profile = $firebaseObject(ref.child('usuarios').child(authData.uid).child('profile'));
	  		
		} else {
	      
	      if(Aut.usuario && Aut.usuario.profile) {
	        Aut.usuario.profile.$destroy();
	      }

	      angular.copy({}, Aut.usuario);
		}
	});
